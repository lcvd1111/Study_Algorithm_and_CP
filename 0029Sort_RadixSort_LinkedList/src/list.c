...
int FindMaximum(LIST *this)
{
	int ret = 0;
	LIST_NODE *currentNode = this->begin;
...
LIST *CountSort_Base_Ten(LIST *this, int base)
{
	...
	for (int i=0 ; i<this->size ; i++){
		if (max < ((currentNode->number)/divisor) % 10){
			max = ((currentNode->number)/divisor) % 10;
		}
	...
	for (int i=0 ; i<this->size ; i++){
		countingArray[((currentNode->number)/divisor)%10] += 1;
		currentNode = currentNode->next;
	}

	//Calculate the cumulative sum of counting array.
	for (int i=0 ; i<9 ; i++){
	...
	for (int i=0 ; i<this->size ; i++){
		countingArray[((currentNode->number)/divisor)%10] -= 1;
		memcpy(&(outputArray[countingArray[((currentNode->number)/divisor)%10]]),
				currentNode,
				sizeof(LIST_NODE));
...
LIST *LIST_METHOD_Sort(LIST *this)
{
	...
	if (FindDigitsNum(12345) != 5){
		PRINTF_ERROR("ERROR: FindDigitsNum() Failed.\n");
		return NULL;
	}
